...
extern struct sockaddr_in s_inn;
extern int f;
extern int trace;
extern int verbose;
extern int rexmtval;
...
size = readit ( file , & dp , -abs(convert) ) ;
if ( size < 0 ) {
break ;
}
...
n = sendto ( f , dp , size + 4 , 0 , (  struct sockaddr * ) & s_inn , sizeof ( s_inn )  
) ;
if ( n != size + 4 ) goto abort ;
...
fromlen = sizeof ( from )  
;
n = recvfrom ( f , ackbuf , sizeof ( ackbuf )  
...
ap_block = ntohs ( ap -> th_u . tu_block ) ;
if ( ap_opcode == 05 ) {
goto abort ;
}
...
send_ack:
  ...
  for ( ; ; ) {
   ...
   s_inn.sin_port = from.sin_port;
   if (trace)
    tpacket("received", dp, n);

   dp->th_opcode = ntohs(dp->th_opcode);
   dp->th_u.tu_block = ntohs(dp->th_u.tu_block);
   if (dp->th_opcode == 05) {
