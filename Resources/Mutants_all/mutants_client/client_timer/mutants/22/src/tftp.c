...
extern union sock_addr peeraddr;
extern int f;
extern int trace;
extern int verbose;
extern int rexmtval;
...
void tftp_sendfile(int fd, const char *name, const char *mode)
{
    ...
    do {
        if (is_request) {
            size = makerequest(02, name, dp, mode) - 4;
        } else {
        ...
        if (trace)
           ...
           n = sendto(f, dp, size + 4, 0,
                   &peeraddr.sa, (((union sock_addr*)&peeraddr)->sa.sa_family == 2 ? (sizeof(struct sockaddr_in)) : (sizeof(union sock_addr))));
        if (n != size + 4) {
            perror("tftp: sendto");
            goto abort;
        }
        ...
        for (;;) {
            ...
            sa_set_port(&peeraddr, (((union sock_addr*)&from)->sa.sa_family == 2 ? ((union sock_addr*)&from)->si.sin_port : ((union sock_addr*)&from)->s6.sin6_port));
...
void tftp_recvfile(int fd, const char *name, const char *mode)
{
    ...
    do {
        if (firsttrip) {
            size = makerequest(01, name, ap, mode);
            firsttrip = 0;
        } else {
      ...
      send_ack:
        ...
        if (sendto(f, ackbuf, size, 0, &peeraddr.sa,
                   (((union sock_addr*)&peeraddr)->sa.sa_family == 2 ? (sizeof(struct sockaddr_in)) : (sizeof(union sock_addr)))) != size) {
            alarm(0);
            perror("tftp: sendto");
            goto abort;
        }
  ...
  abort:
    ...
    (void)sendto(f, ackbuf, 4, 0, (struct sockaddr *)&peeraddr,
                 (((union sock_addr*)&peeraddr)->sa.sa_family == 2 ? (sizeof(struct sockaddr_in)) : (sizeof(union sock_addr))));
    write_behind(file, convert);
    fclose(file);
    stopclock();
    if (amount > 0)
...
static void nak(int error, const char *msg)
{
    ...
    if (sendto(f, ackbuf, length, 0, &peeraddr.sa,
               (((union sock_addr*)&peeraddr)->sa.sa_family == 2 ? (sizeof(struct sockaddr_in)) : (sizeof(union sock_addr)))) != length)
        perror("nak");
}
...
static void tpacket(const char *s, struct tftphdr *tp, int n)
{
    ...
    case 02:
        n -= 2;
        file = cp = (char *)&(tp->th_u.tu_stuff);
        cp = strchr(cp, '\0');
        printf("<file=%s, mode=%s>\n", file, cp + 1);
        break;

    case 03:
        printf("<block=%d, %d bytes>\n", ntohs(tp->th_u.tu_block), n - 4);
        break;

    case 04:
        printf("<block=%d>\n", ntohs(tp->th_u.tu_block));
        break;

    case 05:
        printf("<code=%d, msg=%s>\n", ntohs(tp->th_u.tu_code), tp->th_data);
        break;
    }
}
...
static void startclock(void)
{
    (void)gettimeofday(&tstart, ((void *)0));
}

static void stopclock(void)
{

    (void)gettimeofday(&tstop, ((void *)0));
}
