...
# 92 "/usr/include/i386-linux-gnu/sys/socket.h" 3 4
...
# 1 "/usr/include/arpa/tftp.h" 1 3 4
# 49 "/usr/include/arpa/tftp.h" 3 4
struct tftphdr {
 short th_opcode;
 union {
...
# 1 "/usr/include/i386-linux-gnu/sys/syslog.h" 1 3 4
# 37 "/usr/include/i386-linux-gnu/sys/syslog.h" 3 4
# 1 "/usr/lib/gcc/i486-linux-gnu/4.7/include/stdarg.h" 1 3 4
# 38 "/usr/include/i386-linux-gnu/sys/syslog.h" 2 3 4


# 1 "/usr/include/i386-linux-gnu/bits/syslog-path.h" 1 3 4
# 41 "/usr/include/i386-linux-gnu/sys/syslog.h" 2 3 4
# 169 "/usr/include/i386-linux-gnu/sys/syslog.h" 3 4
...
extern void syslog (int __pri, __const char *__fmt, ...)
     __attribute__ ((__format__ (__printf__, 2, 3)));
# 200 "/usr/include/i386-linux-gnu/sys/syslog.h" 3 4
extern void vsyslog (int __pri, __const char *__fmt, __gnuc_va_list __ap)
     __attribute__ ((__format__ (__printf__, 2, 0)));
# 213 "/usr/include/i386-linux-gnu/sys/syslog.h" 3 4
...
static int peer;
static unsigned long timeout = 1000000;
static unsigned long rexmtval = 1000000;
static unsigned long maxtimeout = ((1 << 5)-1) * 1000000;
static int timeout_quit = 0;
static sigjmp_buf timeoutbuf;
static uint16_t rollover_val = 0;


static char buf[65464 +4];
static char ackbuf[65464 +4];
static unsigned int max_blksize = 65464;

static char tmpbuf[46], *tmp_p;
...
static struct rule *rewrite_rules = ((void *)0);
...
static struct rule *read_remap_rules(const char *file)
{
    ...
    f = fopen(file, "rt");
    if (!f) {
        syslog(3, "Cannot open map file: %s: %m", file);
        exit(66);
    }
...
static void set_socket_nonblock(int fd, int flag)
{
    ...
    err = ((flags = fcntl(fd, 3, 0)) < 0) ||
        (fcntl
         (fd, 4,
          flag ? flags | 04000 : flags & ~04000) < 0);




    if (err) {
        syslog(3, "Cannot set nonblock flag on socket: %m");
        exit(71);
    }
}
...
static void pmtu_discovery_off(int fd)
{
    ...
    setsockopt(fd, IPPROTO_IP, 10, &pmtu, sizeof(pmtu));
...
{
                        unsigned long timeout_s = *timeout_us_p/1000000;
                        int rv;
                 alarm(timeout_s);
...
static struct option long_options[] = {
    ...
    { "create", 0, ((void *)0), 'c' },
    { "secure", 0, ((void *)0), 's' },
    { "permissive", 0, ((void *)0), 'p' },
    { "verbose", 0, ((void *)0), 'v' },
    { "verbosity", 1, ((void *)0), OPT_VERBOSITY },
    { "version", 0, ((void *)0), 'V' },
    { "listen", 0, ((void *)0), 'l' },
    { "foreground", 0, ((void *)0), 'L' },
    { "address", 1, ((void *)0), 'a' },
    { "blocksize", 1, ((void *)0), 'B' },
    { "user", 1, ((void *)0), 'u' },
    { "umask", 1, ((void *)0), 'U' },
    { "refuse", 1, ((void *)0), 'r' },
    { "timeout", 1, ((void *)0), 't' },
    { "retransmit", 1, ((void *)0), 'T' },
    { "port-range", 1, ((void *)0), 'R' },
    { "map-file", 1, ((void *)0), 'm' },
    { "pidfile", 1, ((void *)0), 'P' },
    { ((void *)0), 0, ((void *)0), 0 }
};
...
int main(int argc, char **argv)
{
    ...
    openlog(__progname, 0x01 | 0x08, (3<<3));

int port =6106 ;
int sockfd;

struct sockaddr_in serv_addr, sn;


  sockfd = socket(2, SOCK_DGRAM, 0);
...
# 492 "tftpd.c"
    sn.sin_port = 0;
        ...
        from.sin_family = 2;
         ...
         peer = socket(2, SOCK_DGRAM, 0);
 if (peer < 0) {
  syslog(3, "socket: %m\n");
  exit(1);
 }

 if (bind(peer, (struct sockaddr*)&serv_addr,sizeof(serv_addr)) < 0) {
  syslog(3, "1234bind: %m\n");
                printf("bind error\n");
  exit(1);
 }
...
int tftp(struct tftphdr *tp, int size)
{
    ...
    char *cp, *end;
    int argn, ecode;
    const struct formats *pf = ((void *)0);
    char *origfilename;
    char *filename, *mode = ((void *)0);
    const char *errmsgptr;
    u_short tp_opcode = ntohs(tp->th_opcode);

    char *val = ((void *)0), *opt = ((void *)0);
    char *ap = ackbuf + 2;

    ((struct tftphdr *)ackbuf)->th_opcode = htons(6);

    origfilename = cp = (char *)&(tp->th_u.tu_stuff);
    argn = 0;
    ...
    while (cp < end && *cp) {
        ...
        if (*cp) {
            nak(4, "Request not null-terminated");
            exit(0);
        }
        ...
        if (argn == 1) {
            ...
            if (verbosity >= 1) {
                tmp_p = (char *)inet_ntop(from.sa.sa_family, (((union sock_addr*)&from)->sa.sa_family == 2 ? (void *)&((union sock_addr*)&from)->si.sin_addr : (void *)&((union sock_addr*)&from)->s6.sin6_addr),
                                          tmpbuf, 46);
                if (!tmp_p) {
                    tmp_p = tmpbuf;
                    strcpy(tmpbuf, "???");
                ...
                if (filename == origfilename
                    ...
                    syslog(5, "%s from %s filename %s\n",
                           tp_opcode == 02 ? "WRQ" : "RRQ",
                           tmp_p, filename);
                else
                    syslog(5,
    ...
    if (ap != (ackbuf + 2)) {
        ...
        if (tp_opcode == 02)
            (*pf->f_recv) (pf, ((void *)0), 0);
        else
            (*pf->f_send) (pf, ((void *)0), 0);
    }
    exit(0);
}
...
{
    struct options *po;
    char retbuf[(sizeof(uintmax_t) * 8 / 3 + 3)];
    char *p = *ap;
 ...
 return;
    ...
    for (po = options; po->o_opt; po++)
        if (!strcasecmp(po->o_opt, opt)) {
            if (po->o_fnc(&v)) {
  optlen = strlen(opt);
  retlen = sprintf(retbuf, "%""ll" "u", v);

                if (p + optlen + retlen + 2 >= ackbuf + sizeof(ackbuf)) {
                    nak(8, "Insufficient space for options");
                    exit(0);
                }
  ...
  p += retlen+1;
                ...
                nak(8, "Unsupported option(s) requested");
                exit(0);
            }
            break;
        }
...
static int rewrite_macros(char macro, char *output)
{
    ...
    switch (macro) {
    case 'i':
        p = (char *)inet_ntop(from.sa.sa_family, (((union sock_addr*)&from)->sa.sa_family == 2 ? (void *)&((union sock_addr*)&from)->si.sin_addr : (void *)&((union sock_addr*)&from)->s6.sin6_addr),
                              tb, 46);
        if (output && p)
            strcpy(output, p);
        if (!p)
    ...
    case 'x':
        if (output) {
            if (from.sa.sa_family == 2) {
                sprintf(output, "%08lX",
                    (unsigned long)ntohl(from.si.sin_addr.s_addr));
                l = 8;

            } else {
                unsigned char *c = (unsigned char *)(((union sock_addr*)&from)->sa.sa_family == 2 ? (void *)&((union sock_addr*)&from)->si.sin_addr : (void *)&((union sock_addr*)&from)->s6.sin6_addr);
                p = tb;
                for (l = 0; l < 16; l++) {
                    sprintf(p, "%02X", *c);
                    c++;
      ...
      const struct formats *pf, const char **errmsg)
{
    ...
    *errmsg = ((void *)0);
    ...
    wmode = 01 | (cancreate ? 0100 : 0) | (pf->f_convert ? 0 : 0);
    rmode = 00 | (pf->f_convert ? 0 : 0);





    fd = open(filename, mode == 01 ? rmode : wmode, 0666);
    if (fd < 0) {
    ...
    if (lock_file(fd, mode != 01))
 exit(0);

    if (mode != 01) {
        if (!unixperms && (stbuf.st_mode & (0400 >> 6)) == 0) {
...
static void tftp_sendfile(const struct formats *pf, struct tftphdr *oap, int oacklen)
{
    ...
    if (oap) {
        timeout = rexmtval;
        (void)__sigsetjmp (timeoutbuf, 1);
      oack:
        r_timeout = timeout;
        if (send(peer, oap, oacklen, 0) != oacklen) {
            syslog(4, "tftpd: oack: %m\n");
            goto abort;
        }
        for (;;) {
            n = recv_time(peer, ackbuf, sizeof(ackbuf), 0, &r_timeout);
            if (n < 0) {
                syslog(4, "tftpd: read: %m\n");
                goto abort;
            }
            ap = (struct tftphdr *)ackbuf;
            ap_opcode = ntohs((u_short) ap->th_opcode);
            ap_block = ntohs((u_short) ap->th_u.tu_block);
    ...
    do {
        ...
        if (size < 0) {
            nak((*__errno_location ()) + 100, ((void *)0));
            goto abort;
        }
        dp->th_opcode = htons((u_short) 03);
        dp->th_u.tu_block = htons((u_short) block);
        timeout = rexmtval;
        (void)__sigsetjmp (timeoutbuf, 1);

        r_timeout = timeout;
        if (send(peer, dp, size + 4, 0) != size + 4) {
            syslog(4, "tftpd: write: %m");
            goto abort;
        }
...
ap = (  struct tftphdr * ) ackbuf ;
;
ap -> th_opcode = htons ( (  u_short ) 04 ) ;
ap -> th_u . tu_block = htons ( (  u_short ) block ) ;
acksize = 4 ;
oap = ( (  void * ) 0 ) ;
}
...
for ( ; ; ) 
{
n = recv_time ( peer , dp , 65464 + 4 , 0 , & r_timeout ) ;
if ( n < 0 ) {
goto abort ;
}

dp_opcode = ntohs ( (  u_short ) dp -> th_opcode ) ;
dp_block = ntohs ( (  u_short ) dp -> th_u . tu_block ) ;
if ( dp_opcode == 05 ) goto abort ;
...
if ( dp_block == ( block - 1 ) ) goto send_ack ;
...
timeout_quit = 1 ;
n = recv_time ( peer , buf , sizeof ( buf )  
, 0 , & timeout ) ;
timeout_quit = 0 ;
dp_opcode = ntohs((u_short) ((struct tftphdr *)buf)->th_opcode);
dp_block = -abs(ntohs((u_short) ((struct tftphdr *)buf)->th_u.tu_block));
if ( n >= 4 && dp_opcode == 03 && block == dp_block ) {
(  void ) send ( peer , ackbuf , 4 , -1 ) ;
}
...
static void nak(int error, const char *msg)
{
    ...
    if (error >= 100) {
        ...
        if ((unsigned)error >= (sizeof(errmsgs)/sizeof(const char *)))
            error = 0;
    ...
    tp->th_u.tu_code = htons((u_short) error);

    length = strlen(msg) + 1;
    memcpy(tp->th_data, msg, length);
    length += 4;
    ...
    if (verbosity >= 2) {
        ...
        syslog(6, "sending NAK (%d, %s) to %s",
               error, tp->th_data, tmp_p);
    }
